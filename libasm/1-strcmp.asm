...
.next_char:
    mov al, byte [rdi]    ; Load byte from the first string (rdi points to the address of the first string)
    mov dl, byte [rsi]    ; Load byte from the second string (rsi points to the address of the second string)
    cmp al, dl            ; Compare the two bytes
    jne .diff             ; If not equal, jump to the diff label
    test al, al           ; Check if we’ve hit '\0'
    je .done              ; If '\0', the strings are equal (end of string)
